...
PHASE_NAME = {TRAIN: 'training', VALID: 'validation', TEST: 'test'}
...
def reduce_mean_masked(input_tensor, is_valid, axis=None, keepdims=False, try_fast=True):
    ...
    n_valid = tf.count_nonzero(is_valid, axis=axis, keepdims=keepdims, dtype=input_tensor.dtype)
...
def mean_stdev_masked(input_tensor, is_valid, items_axis, dimensions_axis, fixed_ref=None):
    ...
    n_valid = tf.count_nonzero(is_valid, axis=items_axis, keepdims=True, dtype=input_tensor.dtype)

    sum_of_squared_deviations = reduce_sum_masked(
        tf.square(centered), is_valid, axis=[items_axis, dimensions_axis], keepdims=True)

    stdev = tf.sqrt(tf.div_no_nan(sum_of_squared_deviations, n_valid) + 1e-10)
    return mean, stdev
...
def static_batch_size(tensor):
    return static_shape(tensor)[0]
...
def dynamic_n_channels(tensor):
    return tf.shape(tensor)[channel_axis()]
...
def dynamic_image_shape(tensor):
    if data_format() == 'NHWC':
        return tf.shape(tensor)[1:3]
    else:
        return tf.shape(tensor)[2:4]
...
def get_channel(image, i_channel):
    if data_format() == 'NHWC':
        return image[:, :, i_channel]
    else:
        return image[i_channel, :, :]


def count_trainable_params():
    return sum(np.prod(static_shape(var)) for var in tf.trainable_variables())


def py_func_with_shapes(
        func, inp=None, output_types=None, output_shapes=None, stateful=True, name=None):
    result = tf.py_func(func, inp, output_types, stateful, name)
...
def py_func_with_auto_shapes_and_types(
        func, inp=None, Tout=None, shapes_out=None, stateful=True, name=None, example_inp_val=(),
        exchangable=None):
    """Automatically determines and sets the types and shapes of the resulting py_func

    Args:
        func: Python function taking numpy arrays and producing a list of numpy arrays
        inp: input tensors to the function
        Tout: output types for the arrays as TF dtypes
        shapes_out: shape of the output arrays
        stateful: whether the same inputs can lead to different outputs (opposite of deterministic)
        name:
        example_inp_val:
        exchangable: whether the samples produced by a stateful function are exchangeable so that
        one can be sampled at the start to determine the sizes and types
    ...
    if Tout is None or shapes_out is None:
        if exchangable:
            sample_output = func(*example_inp_val)
            shapes_out, Tout = get_shapes_and_tf_dtypes(sample_output)
        elif not inp:
            sample_output = func(*example_inp_val)
            shapes_out, Tout = get_shapes_and_tf_dtypes(sample_output)
            ...
            raise Exception(
                'Function takes input and is not exchangable, '
                'therefore shapes and types must be specified explicitly.')

    return py_func_with_shapes(func, inp, Tout, shapes_out, stateful, name)
...
def get_or_create_counter(name):
    ...
    try:
        ...
        counter = attrdict.AttrDict(
            name=name, var=var, reset_op=tf.assign(var, 0), increment_op=tf.assign_add(var, 1))
...
def gradients_with_loss_scaling(loss, variables, loss_scale=128):
    ...
    """
    return [tf.cast(grad, tf.float32) / loss_scale if grad is not None else None
            for grad in tf.gradients(loss * loss_scale, variables, gate_gradients=True)]
...
def in_name_scope(default_name):
    ...
    def wrapper(f):
        ...
        def wrapped(*args, scope=None, **kwargs):
            with tf.name_scope(scope, default_name):
                return f(*args, **kwargs)
...
def make_pretrained_weight_loader(pretrained_path, loaded_scope, checkpoint_scope, excluded_parts):
    ...
    savers = [tf.train.Saver(var_list=d) for d in new_var_dicts]
...
def scalar_dict_to_summary(dic):
    return tf.Summary(value=[tf.Summary.Value(tag=t, simple_value=v) for t, v in dic.items()])
