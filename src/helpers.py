...
def train(
    ...
    t_train = graph_fn(tfu.TRAIN, reuse=tf.AUTO_REUSE)
    ...
    t_valid = (graph_fn(tfu.VALID, reuse=tf.AUTO_REUSE) if validation else None)
...
def build_input_batch(
        ...
        drop_remainder=None, rng=None, max_unconsumed=256, n_completed_steps=0, n_total_steps=None,
        n_test_epochs=1):
    if shuffle is None:
...
def make_session_creator(checkpoint_dir=None, load_path=None, init_fn=None):
    ...
    if load_path:
        if load_path.endswith('.index') or load_path.endswith('.meta'):
            load_path = os.path.splitext(load_path)[0]
        if not os.path.isabs(load_path) and checkpoint_dir:
            load_path = os.path.join(checkpoint_dir, load_path)
        return tf.train.ChiefSessionCreator(
    ...
    return tf.train.ChiefSessionCreator(config=config, scaffold=scaffold)
