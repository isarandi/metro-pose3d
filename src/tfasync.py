...
class AsyncMultiHook(MultiHook):
    ...
    def is_managed_hook(hook):
        return (isinstance(hook, CoroutineHook)
                or (isinstance(hook, _PeriodicHookWrapper)
...
class _PeriodicHookWrapper(tf.train.SessionRunHook):
    ...
    def before_run(self, run_context):
        ...
        if self._should_trigger:
            ...
            hook_run_args = tf.train.SessionRunArgs([])
        ...
        if self._step_tensor is not None:
            ...
            return tf.train.SessionRunArgs(fetches, hook_run_args.feed_dict, hook_run_args.options)
